...
sData * sData::openDataBase(sirius * s, QString filename)
{
	if(fDb) return NULL;
	return new sData(s, filename);
}

sData::sData(sirius * s, QString filename) :
	QObject(s, QString("db%1").arg(filename))
{
	fSirius = s;
	fDb = NULL;	
...
{
	if(!fDb) return _u("База данных не открыта.");
	QString res = _u(sqlite3_errmsg(fDb));
	return res;
}
