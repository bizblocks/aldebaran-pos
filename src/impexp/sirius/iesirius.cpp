...
void sServer::readData(Q3Socket * s)
{    
...
QString expSirius::errorMsg(int e)
{
    ...
    case Q3Socket::ErrConnectionRefused: res = QObject::tr("Connection refused"); break;
    case Q3Socket::ErrHostNotFound: res = QObject::tr("Host not found"); break;
...
bool expSirius::isOpened()
{
    if(!fSocket)
        return FALSE;
    return fSocket->state()==Q3Socket::Connected;
}
...
{
    if(row<0 || row>=fData.count()) return FALSE;
    fields.clear();
    if(fCmd=='O')
    {
...
{
    if(fCurrent<0 || fCurrent>=fData.count())
        return QVariant();
