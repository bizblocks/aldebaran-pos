...
bool expVMart::open(QString host, int port, QString db, QString user, QString pass, QString prefix)
{
    ...
    fDb = new QSqlDatabase(QSqlDatabase::addDatabase("QMYSQL3", fDbName));
    if(!fDb)
    {
...
{
    fDb = new QSqlDatabase(QSqlDatabase::database(fDbName));
    if(!fDb || !fDb->isOpen())
    {
        setError(VMART_NOTOPENED);
        emit error();
...
void impVMart::checkNewOrder()
{
    ...
    Q3SqlSelectCursor orders(orders_users_query, *fDb);
    if(orders.first())
    {
        ...
        fData["0"] = values;
        Q3SqlCursor items(QString("%1order_item").arg(fPrefix), TRUE, *fDb);
