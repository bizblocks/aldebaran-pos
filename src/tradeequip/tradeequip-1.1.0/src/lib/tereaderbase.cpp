/****************************************************************************
** $Id: tereaderbase.cpp,v 1.9 2006/06/20 05:06:55 red75 Exp $
**
...
bool TEReaderBase::startDriver() // open port, then start polling it
{
    if (!openPort())
    {
...
bool TEReaderBase::stopDriver() // stop polling, then closes port
{
    stopPoll();
    if (!closePort())
    {
