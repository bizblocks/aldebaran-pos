...
bool TEIniFile::useSection(const QString &section)
{
    ...
    mi = QMapIterator<QString, type_ValueList>(SectionList);
    QMap<QString, type_ValueList>::iterator it  = SectionList.find(section);
    if(it == SectionList.end())
    {
        type_ValueList empty;
        SectionList[section]=empty;
        it=SectionList.find(section);
    }
    return true;
}
...
{
    if (mi.hasNext()) return (QString&)def;
    SectionListDef[m_sCurSection][name]=def;
    return (mi.value().contains(name)) ? mi.value()[name] : def;
}
...
{
    return mi.value().find(name)!=mi.value().end() ||
...
{
    if (!mi.hasNext()) // It checks if iterator is not initialized.
        return;
    mi.value()[name]=value;
}
...
void TEIniFile::setData(const QString &name, const QString &value)
{
    if (!mi.hasNext()) // It checks if iterator is not initialized.
        return;
    mi.value()[name]=qstrFromData(value);
}
...
void TEIniFile::setNumber(const QString &name, const int value, const int base)
{
    if (!mi.hasNext()) // It checks if iterator is not initialized.
        return;
    mi.value()[name]=qstrFromInt(value,base);
}
...
void	TEIniFile::setBool(const QString &name, const bool value)
{
    if (!mi.hasNext()) // It checks if iterator is not initialized.
        return;
    mi.value()[name]=qstrFromBool(value);
}
...
void	TEIniFile::setColor(const QString &name, const QColor& color)
{
    if (!mi.hasNext()) // It checks if iterator is not initialized.
        return;
    mi.value()[name]=qstrFromColor(color);
}
...
void	TEIniFile::setRect(const QString &name, const QRect& rect)
{
    if (!mi.hasNext()) // It checks if iterator is not initialized.
        return;
    mi.value()[name]=qstrFromRect(rect);
}
...
bool TEIniFile::update()
{
    ...
    QMapIterator<QString,type_ValueList> it1(SectionList);
    while(it1.hasNext())
    {
        it1.next();
    ...
    QMapIterator<QString,bool> it(secProcd);
    while(it.hasNext())
    {
        it.next();
        QString sec=it.key();
...
{
    QStringList sl;
    QMapIterator<QString, type_ValueList> it(SectionListDef);
    while(it.hasNext())
    {
        it.next();
        type_ValueListIterator it1(it.value());
        while(it1.hasNext())
        {
            it1.next();
            sl << it.key()+"->"+it1.key();
