...
ShtrihFR::ShtrihFR() : TECashRegisterBase("/dev/ttyS0",0)
{
	init();
}

ShtrihFR::ShtrihFR(QString pn, Q_ULONG mn) : TECashRegisterBase(pn, mn)
{
	init();
	setPortDevice(pn);
	setMachineNumber(mn);
...
Q_ULONG ShtrihFR::readMachineNumber()
{
	...
	if (internalReadState(&st)==RSP_OK) setMachineNumber(from4Bytes(st.serialNumber));
	else setMachineNumber(0);
  ...
  makeHelper; // This macro incorporates OClHelper, which automagically opens/closes port as needed.
    ...
    { //CR waits for continue print command
      ...
      if (res)
		...
		res = internalPrintLine( checkHeader(), true, true);
